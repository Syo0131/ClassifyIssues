...
**The dev spec carries traceability and phasing.** Each `functionalRequirements[]` entry has `acceptanceCriteria: string[]`; each `modules[]` entry has `id` (`M-01`…), `phase` (1 = MVP), and `requirementIds` (the RF-xx it delivers); `spec.phases[]` names the delivery phases. `validateDevelopmentSpec()` (`lib/ai.ts`) defaults all of these for old/partial specs and raises reviewer warnings for `must` requirements left uncovered or without acceptance criteria (`validateTraceability`, `normalizePhases`). All fields are optional at read time — pre-existing specs render unchanged, no DB migration.

**Two Gemini calls per dev analysis.** After the draft is generated and validated, `critiqueDevelopmentSpec()` runs a second Gemini profile (`'critique'`) — a skeptical delivery-lead review that returns pointed `estimateAdjustments` (re-clamped through `clampHours` + `min ≤ likely ≤ max`), plus added risks/questions/assumptions, and a verdict. It **never throws**: no key, `DEV_CRITIQUE_PASS=off`, or any failure returns the draft untouched with a warning. This adds ~15-20s on top of generation, all inside `runDevAnalysisInBackground` (not on the request path). `withRetry` was also fixed here — it now matches the Gemini SDK's string-encoded status codes and network errors, so the backoff actually fires.

**Development analysis runs in the background.** The Gemini calls take ~30-40s total, too long to block on. `POST /api/analyze` for a `desarrollo` ticket creates the row immediately with a placeholder analysis (`source: 'pending'`, `spec` NULL), returns `202`, and the client is redirected straight to `/dashboard`. `runDevAnalysisInBackground()` (in `lib/dev-analysis.ts`) is fired without `await` and fills the spec in via `applyDevTicketAnalysis()`. **`spec` NULL on a `desarrollo` ticket means "still analyzing"** — that's the pending sentinel the UI polls on (the staff ticket detail auto-refreshes every 4s until it arrives). This relies on the standalone Node server being long-lived; a restart mid-analysis leaves the ticket pending forever. Runbook to reprocess: call `runDevAnalysisInBackground(id, rawText, brief)` for any `desarrollo` row where `spec IS NULL`, or resubmit. If this becomes load-bearing, move it to a real job queue.
...
The spec deliberately contains **no money**. `lib/budget.ts` derives cost from hours × `DEV_HOURLY_RATE` at render time and the result is never stored, so changing the rate reprices every existing ticket without re-calling Gemini. Budgets are computed server-side and passed into client components as props.

`calculateBudget()` uses **PERT**: per module `expected = (min + 4·likely + max) / 6`, `sd = (max - min) / 6`; the total combines module variances and exposes a `p80` (percentile via `DEV_QUOTE_PERCENTILE`, default 80) alongside `min/likely/max/expected`. **Contingency is dynamic** by default — `computeContingency()` scales it by complexity, estimate spread, and open-question count, and returns a `contingencyRationale` string; setting `DEV_CONTINGENCY_PCT` forces a fixed value instead. `timeline` converts hours → weeks per phase using `DEV_TEAM_SIZE` (1) × `DEV_HOURS_PER_WEEK` (32). Costs round to whole currency units so the printed total equals the sum of the printed lines. The `Budget` type's figure groups (`hours`/`subtotal`/`contingency`/`total`) are all `BudgetFigures` (`{min,likely,max,expected,p80}`), a superset of the old shape.
